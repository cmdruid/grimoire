...
validate() {
  ...
  cmp -s "$JOURNAL/scripts/records.sh" "$root/.records/records.sh" || {
    fail 'deployed records provider differs from its package source'; return 1;
  }
  cmp -s "$BACKLOG/scripts/trackers.sh" "$root/.trackers/trackers.sh" || {
  ...
  cat "$BACKLOG/templates/trackers-readme-block.md" >>"$expected"
  ...
  "$JOURNAL/scripts/standup.sh" repair "$probe" >/dev/null 2>&1 || {
...
FIX="$T/repo"
mkdir -p "$FIX/.records" "$FIX/.trackers"
cp "$JOURNAL/scripts/records.sh" "$FIX/.records/records.sh"
cp "$BACKLOG/scripts/trackers.sh" "$FIX/.trackers/trackers.sh"
