---
name: mailbox
description: "Use when an agent needs to delegate file-work to a sub-agent -- planning, implementing, remediating, testing, reviewing -- and wants the result back WITHOUT polluting its own context, WITHOUT the sub-agent editing a shared git worktree directly (which silently corrupts the tree), and ideally on a different model than the orchestrator (cheap model for grunt work, strong model for hard reasoning). Covers dispatching a model-routed sub-agent that hands its output back out-of-band through a scratch file. Harness-agnostic: a Claude or a Codex agent can use it. Keywords: dispatch, delegate, sub-agent, worktree corruption, scratch file, mailbox, token cost, model routing, context isolation, handoff, git apply."
---
