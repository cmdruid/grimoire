...
U="$T/uninitialized";newroot "$U";no "$SETUP" "$U" "${BASE[@]}" repair
...
R="$T/root";newroot "$R";"$SETUP" "$R" "${BASE[@]}" --apply >/dev/null
printf 'DOOR_CANARY\n'>>"$R/AGENTS.md";printf 'PROMPT_CANARY\n'>>"$R/.spaces/backlog/hooks/debrief.md"
cp "$R/AGENTS.md" "$T/door";cp "$R/.spaces/backlog/hooks/debrief.md" "$T/prompt";cp "$R/.trackers/tasks.tsv" "$T/queue";cp "$R/.trackers/receipts.tsv" "$T/receipts"
...
cmp "$T/door" "$R/AGENTS.md" >/dev/null&&cmp "$T/prompt" "$R/.spaces/backlog/hooks/debrief.md" >/dev/null&&cmp "$T/queue" "$R/.trackers/tasks.tsv" >/dev/null&&cmp "$T/receipts" "$R/.trackers/receipts.tsv" >/dev/null&&pass=$((pass+1))||fail=$((fail+1))
...
for population in removed custom zero;do
  V="$T/population-$population";newroot "$V";"$SETUP" "$V" "${BASE[@]}" --apply >/dev/null
  case "$population" in
    removed)"$SETUP" "$V" "${BASE[@]}" tracker-remove issues >/dev/null;;
    custom)"$SETUP" "$V" "${BASE[@]}" tracker-add decisions >/dev/null;for s in tasks issues feedback routines;do "$SETUP" "$V" "${BASE[@]}" tracker-remove "$s" >/dev/null;done;;
    zero)for s in tasks issues feedback routines;do "$SETUP" "$V" "${BASE[@]}" tracker-remove "$s" >/dev/null;done;;
  esac
  ...
  ok "$SETUP" "$V" "${BASE[@]}" repair;[ "$before_count" -eq "$(find "$V/.trackers" -name '*.tsv' ! -name receipts.tsv|wc -l|tr -d ' ')" ]&&pass=$((pass+1))||fail=$((fail+1))
done

# An injected repair failure can leave only an allowed provider write; retry completes the README.
I="$T/interrupted";newroot "$I";"$SETUP" "$I" "${BASE[@]}" --apply >/dev/null;printf 'bad\n'>"$I/.trackers/trackers.sh";chmod +x "$I/.trackers/trackers.sh";sed 's/## Use the tracker tool/## Drifted tracker tool/' "$I/.trackers/README.md">"$T/readme.drift";mv "$T/readme.drift" "$I/.trackers/README.md"
STOP="$T/stop.sh";printf '%s\n' '#!/bin/sh' '[ "$4" -ne 1 ] || exit 86'>"$STOP";chmod +x "$STOP"
if BACKLOG_SETUP_TEST_AFTER_WRITE="$STOP" "$SETUP" "$I" "${BASE[@]}" repair >/dev/null 2>&1;then fail=$((fail+1));else pass=$((pass+1));fi
cmp "$SOURCE" "$I/.trackers/trackers.sh" >/dev/null&&pass=$((pass+1))||fail=$((fail+1));ok "$SETUP" "$I" "${BASE[@]}" repair

# Malformed data and ownership markers refuse before any repair write.
Q="$T/malformed-queue";newroot "$Q";"$SETUP" "$Q" "${BASE[@]}" --apply >/dev/null;printf 'bad\n'>"$Q/.trackers/tasks.tsv";cp "$Q/.trackers/trackers.sh" "$T/provider.before";no "$SETUP" "$Q" "${BASE[@]}" repair;cmp "$T/provider.before" "$Q/.trackers/trackers.sh" >/dev/null&&pass=$((pass+1))||fail=$((fail+1))
M="$T/malformed-readme";newroot "$M";"$SETUP" "$M" "${BASE[@]}" --apply >/dev/null;printf '\n<!-- backlog:trackers-tool BEGIN -->\n'>>"$M/.trackers/README.md";rm "$M/.trackers/trackers.sh";no "$SETUP" "$M" "${BASE[@]}" repair;[ ! -e "$M/.trackers/trackers.sh" ]&&pass=$((pass+1))||fail=$((fail+1))
...
N="$T/vanished-root";newroot "$N";"$SETUP" "$N" "${BASE[@]}" --apply >/dev/null;printf 'bad\n'>"$N/.trackers/trackers.sh";chmod +x "$N/.trackers/trackers.sh";MOVED="$T/vanished-layer"
if MOVED="$MOVED" BACKLOG_SETUP_TEST_AFTER_PREFLIGHT="$VANISH" "$SETUP" "$N" "${BASE[@]}" repair >"$T/vanished.out" 2>&1;then echo 'FAIL repair recreated vanished root' >&2;fail=$((fail+1));else pass=$((pass+1));fi
...
C="$T/readme-race";newroot "$C";"$SETUP" "$C" "${BASE[@]}" --apply >/dev/null;sed 's/## Use the tracker tool/## Drifted tracker tool/' "$C/.trackers/README.md">"$T/readme.race";mv "$T/readme.race" "$C/.trackers/README.md"
if BACKLOG_SETUP_TEST_BEFORE_README_RENAME="$EDIT" "$SETUP" "$C" "${BASE[@]}" repair >"$T/readme-race.out" 2>&1;then echo 'FAIL concurrent README edit was overwritten' >&2;fail=$((fail+1));else pass=$((pass+1));fi
