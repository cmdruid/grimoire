...
R="$T/root";legacy_owner_tracker_dir="$R/.agents/skilldata/backlog/""trackers";mkdir -p "$R/.records/trackers" "$legacy_owner_tracker_dir";git -C "$R" init -q
legacy_record='LEGACY_RECORD_CANARY_8421';legacy_owner_tracker='LEGACY_WORKSPACE_CANARY_9532';live='LIVE_TRACKER_CANARY_1064'
printf '%s\n' "$legacy_record" > "$R/.records/trackers/old.md"
printf '%s\n' "$legacy_owner_tracker" > "$legacy_owner_tracker_dir/tasks.tsv"
...
has "$R/.records/trackers/old.md" "$legacy_record";has "$legacy_owner_tracker_dir/tasks.tsv" "$legacy_owner_tracker"
has "$combined" "$live";lacks "$combined" "$legacy_record";lacks "$combined" "$legacy_owner_tracker"
...
legacy_owner_path='.agents/skilldata/backlog/'"trackers"
