...
for needle in 'nonempty multi-select' 'Unattended setup' '--trackers <comma-separated-stems>' 'valid only during initialization' 'valid cleanup state';do grep -qF -- "$needle" "$B/verbs/setup.md"&&pass=$((pass+1))||fail=$((fail+1));done
