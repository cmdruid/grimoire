#!/usr/bin/env bash
# record-mint-test.sh — mktemp fixture smoke for record-mint.sh.
# Nothing touches the library tree except reading the script and templates.
set -eu
DIR="$(cd "$(dirname "$0")" && pwd)"
SKILL="$(cd "$DIR/../.." && pwd)"
MINT="$SKILL/scripts/record-mint.sh"
...
today="$(date +%Y-%m-%d)"
TMP="$(mktemp -d "${TMPDIR:-/tmp}/record-mint-test.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
echo "record-mint-test: $pass passed, $fail failed"
[ "$fail" -eq 0 ]
