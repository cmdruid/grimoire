---
name: backlog
description: "The records instrument — the single collection front-door for a project's `.records/` stores. Owns the trackers and their wire formats, typed counter IDs, completion (`done` + the done log), the ticket escalation layer (optional remote mirror), curation, and the deployed record schema. Capture by kind — `/backlog task|bug|issue|note|feedback` — plus `debrief` (sweep finished work), `curate` (hygiene), `ticket`/`promote`/`close` (escalate to the human and resolve), `sync` (the mirror). Tool-like: it captures and completes; what a captured signal means is the improvement loop's judgment, not made at capture time. Use when the user runs `/backlog ...`, files/sweeps/curates a follow-up, marks work done, or escalates to the human."
---
