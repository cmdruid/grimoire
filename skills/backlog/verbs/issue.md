...
2. **Find or create the Issues tracker**: if `records.sh` is executable,
   `records.sh --root <root> --records-root <records-root-relative> list --type trackers`, title `Issues`; else scan live
   `<agent-records>/trackers/*.md` by H1. When absent,
   `scripts/record-mint.sh mint <root> <records-root-relative> <workspace-relative> trackers "Issues"`.
