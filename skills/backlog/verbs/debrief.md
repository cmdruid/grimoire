...
7. Invoke only API `create` or `update`. Commit all changed queue paths once with
   ...
   its parent before custody ends. Remember this successful boundary within the current custody
   chain; do not create a durable cursor or alternate save-state buffer.
