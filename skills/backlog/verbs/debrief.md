...
4. **Close what completed**: records this work finished get
   ...
   (no ledger line for the line-item).
...
- Candidates gathered from status, the resolved `<base>` diff, and the conversation; each
  routed write-only; completed records closed via stamp (ledger line only when `records.sh`
  ran); completed tracker lines match the contract's completed form and were touched; one
  scoped commit on the probed tree; sweep reported.
