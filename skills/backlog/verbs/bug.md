...
The load-bearing rule: **`.records/bugs/` is a store, not a work queue.** You file a report there and
...
- The linking actionable item lives in `.records/tasks.md` (or an `.records/issues.md` entry if
...
4. **Write the report** to `.records/bugs/<YYYY-MM-DD>-<slug>.md` from `templates/bug-report.md`:
...
5. **Link it from an actionable item** — `bugs/` is a store, so the report must be tracked from
   ...
   - **Fixing now?** Make the fix (patch or feature, by size), note the commit in the report's
     Status, then `git mv` the report to `.records/bugs/archive/`.
   - **Deferring?** Add a `.records/tasks.md` line (or an `.records/issues.md` entry if it's a
...
The defect has a report in `.records/bugs/` with a real repro + evidence, that report is **linked from an
