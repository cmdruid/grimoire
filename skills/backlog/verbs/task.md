...
A product/feature follow-up: something someone should **build or change**. One concrete
sentence, cold-actionable.

1. Resolve both homes (SKILL.md).
2. **Find or create the Backlog tracker**: if `records.sh` is executable,
   `records.sh --root <root> --records-root <records-root-relative> list --type trackers` and match the title `Backlog`; else scan
   live `<agent-records>/trackers/*.md` by H1. When absent,
   `scripts/record-mint.sh mint <root> <records-root-relative> <workspace-relative> trackers "Backlog"`.
3. **Append one line** under `## Items`, newest last, in the contract's live tracker-line
   ...
   `→ <dir>/<file>.md` when one exists. Keep it one line — detail that doesn't fit belongs
   in a linked record.
