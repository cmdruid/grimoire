...
Keep `.agents/backlog/TASKS.md` sharp. This verb **reshapes** the existing list — it does not capture new
...
`/foreman tune`'s job, not this verb's.
...
(`/backlog debrief`), or draining system-relevant signal into doctrine (`/foreman tune`). Don't invoke
...
- Tasks: `<root>/.agents/backlog/TASKS.md`. If it doesn't exist, there's nothing to curate — say so and stop.
...
5. **Commit (standalone only).** Invoked **standalone**, scoped-commit the curated list via
   `scripts/scoped-commit.sh <root> "Tasks: curate" .agents/backlog/TASKS.md`, then run the host doc-linter.
...
- **`/foreman tune`** — the periodic system-wide drain/audit that folds captured signal into
...
`.agents/backlog/TASKS.md` is sharper — reordered by relevance, vague items split/filled, dead items weeded with
