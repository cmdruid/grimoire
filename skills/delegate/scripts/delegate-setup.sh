...
rel="$skilldata/delegate/hooks/byproducts.md";[ -z "${DELEGATE_SETUP_TEST_DEST_REL:-}" ]||rel="$DELEGATE_SETUP_TEST_DEST_REL";case "$rel" in "$skilldata/delegate/hooks/byproducts.md");;*)die "destination escapes delegate ownership: $rel";;esac;check "${rel%/*}";dst="$root/$rel";[ ! -L "$dst" ]||die "destination is a symlink: $dst";[ ! -e "$dst" ]||[ -f "$dst" ]||die "destination is not a regular file: $dst"
...
[ -z "${DELEGATE_SETUP_TEST_AFTER_PREFLIGHT:-}" ]||"$DELEGATE_SETUP_TEST_AFTER_PREFLIGHT" "$root" "$skilldata"
