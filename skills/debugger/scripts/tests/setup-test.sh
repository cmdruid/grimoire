...
SUM="$(cksum "$ROOT/.spaces/debugger/templates/bugs.md")"; bash "$SETUP" --write-only "$ROOT" >"$OUT"
eq "incumbent preserved" "$SUM" "$(cksum "$ROOT/.spaces/debugger/templates/bugs.md")"
...
no "legacy not adopted" "$LEG/.spaces/debugger/templates/bugs.md"
...
RACE="$TMP/race"; mkdir -p "$RACE/.spaces/debugger" "$RACE/elsewhere"; HOOK="$TMP/preflight.sh"
...
has "partial reports completed path" 'created=.spaces/debugger/templates/bugs.md' "$OUT"
mv "$PART/.spaces/debugger/templates" "$PART/bad-link"; mv "$PART/held-templates" "$PART/.spaces/debugger/templates"
bash "$SETUP" --write-only "$PART" >"$OUT"
eq "partial rerun converges" 3 "$(find "$PART/.spaces/debugger" -type f | wc -l | tr -d ' ')"
...
if DEBUGGER_SETUP_TEST_DEST_REL='.spaces/notepad/templates/bugs.md' bash "$SETUP" --write-only "$BOUND" >/dev/null 2>&1; then echo 'FAIL: sibling dest accepted' >&2; fail=$((fail+1)); else pass=$((pass+1)); fi
