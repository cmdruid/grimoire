...
dest_rel(){ case "$1" in templates/reports.md)printf '%s\n' "$ws/auditor/templates/reports.md";;rules/*)printf '%s\n' "$ws/auditor/doctrine/test/workflows/audit/$1";;esac;}
while IFS= read -r rel;do [ -n "$rel" ]||continue;case "$rel" in templates/reports.md|rules/*.md);;*)die "asset is not declared for project deployment: $rel";;esac;src="$base/$rel";[ -f "$src" ]&&[ ! -L "$src" ]||die "bad bundled asset: $src";case "$rel" in templates/*)schema_free "$src"||die "bundled project template selects a schema: $src";;esac;dstrel="$(dest_rel "$rel")";[ -z "${AUDITOR_SETUP_TEST_DEST_REL:-}" ]||dstrel="$AUDITOR_SETUP_TEST_DEST_REL";case "$dstrel" in "$ws/auditor/templates/reports.md"|"$ws/auditor/doctrine/test/workflows/audit/rules/"*.md);;*)die "destination escapes auditor ownership: $dstrel";;esac;check "${dstrel%/*}";dst="$root/$dstrel";[ ! -L "$dst" ]||die "destination is a symlink: $dst";[ ! -e "$dst" ]||[ -f "$dst" ]||die "destination is not a regular file: $dst";if [ -e "$dst" ];then case "$rel" in templates/*)schema_free "$dst"||die "project template selects a schema: $dst";;esac;fi;done <<EOF
$assets
EOF
if [ ! -e "$root/$ws/auditor/templates/reports.md" ];then for old in "$root/$rr/templates/auditor/reports.md" "$root/$rr/templates/reports.md";do [ ! -L "$old" ]&&[ ! -e "$old" ]||die "legacy template found; run /auditor migrate $old";done;fi
if [ ! -e "$root/$ws/auditor/doctrine/test/workflows/audit/GUIDE.md" ];then for old in "$root/.handbook/test/workflows/audit" "$root/$rr/doctrine/test/workflows/audit" "$root/$rr/doctrine/audit";do [ ! -L "$old" ]&&[ ! -e "$old" ]||die "legacy rubric found; run /auditor migrate $old";done;fi
[ -z "${AUDITOR_SETUP_TEST_AFTER_PREFLIGHT:-}" ]||"$AUDITOR_SETUP_TEST_AFTER_PREFLIGHT" "$root" "$ws"
while IFS= read -r rel;do [ -n "$rel" ]||continue;src="$base/$rel";dstrel="$(dest_rel "$rel")";dst="$root/$dstrel";if [ -f "$dst" ]&&[ ! -L "$dst" ];then printf 'preserved=%s\n' "$dstrel";continue;fi;ensure "${dstrel%/*}";check "${dstrel%/*}";[ ! -L "$dst" ]&&[ ! -e "$dst" ]||die "destination changed after preflight: $dst";cp "$src" "$dst";n=$((n+1));printf 'created=%s\n' "$dstrel";[ -z "${AUDITOR_SETUP_TEST_AFTER_WRITE:-}" ]||"$AUDITOR_SETUP_TEST_AFTER_WRITE" "$root" "$ws" "$dstrel" "$n";done <<EOF
$assets
EOF
printf 'writes=%s\n' "$n"
