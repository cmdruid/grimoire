...
R="$T/root";mkdir -p "$R";bash "$S" "$R">"$O";rules="$(find "$R/.agents/skilldata/auditor/doctrine/test/workflows/audit/rules" -type f|wc -l|tr -d ' ')";eq "all bundled rules" 13 "$rules";ok test -f "$R/.agents/skilldata/auditor/templates/reports.md";no "seed does not invent GUIDE" "$R/.agents/skilldata/auditor/doctrine/test/workflows/audit/GUIDE.md";no "seed does not invent metrics" "$R/.agents/skilldata/auditor/doctrine/test/workflows/audit/metrics.sh";ok forbidden_clear "$R"
printf '\nCUSTOM\n'>>"$R/.agents/skilldata/auditor/templates/reports.md";sum="$(cksum "$R/.agents/skilldata/auditor/templates/reports.md")";bash "$S" "$R">"$O";eq "incumbent preserved" "$sum" "$(cksum "$R/.agents/skilldata/auditor/templates/reports.md")";ok grep -q writes=0 "$O"
L="$T/legacy";mkdir -p "$L/.records/templates/auditor";printf x>"$L/.records/templates/auditor/reports.md";if bash "$S" "$L">"$O" 2>&1;then f=$((f+1));else p=$((p+1));fi;ok grep -q '/auditor migrate' "$O";no "legacy not adopted" "$L/.agents/skilldata/auditor/templates/reports.md"
U="$T/unsafe";mkdir -p "$U/else" "$U/.agents";ln -s "$U/else" "$U/.agents/skilldata";if bash "$S" "$U">"$O" 2>&1;then f=$((f+1));else p=$((p+1));fi;no "unsafe no write" "$U/else/auditor"
X="$T/exchange";mkdir -p "$X/.agents/skilldata/auditor" "$X/else";H="$T/h";printf '%s\n' '#!/bin/sh' 'mv "$1/$2/auditor" "$1/held"' 'ln -s "$1/else" "$1/$2/auditor"'>"$H";chmod +x "$H";if AUDITOR_SETUP_TEST_AFTER_PREFLIGHT="$H" bash "$S" "$X">"$O" 2>&1;then f=$((f+1));else p=$((p+1));fi;ok grep -q 'symlinked destination parent' "$O";no "exchange no write" "$X/else/templates"
P="$T/partial";mkdir -p "$P";PH="$T/write-hook";printf '%s\n' '#!/bin/sh' '[ "$4" -eq 1 ] && exit 86' 'exit 0'>"$PH";chmod +x "$PH";if AUDITOR_SETUP_TEST_AFTER_WRITE="$PH" bash "$S" "$P">"$O" 2>&1;then f=$((f+1));else p=$((p+1));fi;ok grep -q 'created=.agents/skilldata/auditor/templates/reports.md' "$O";eq "partial leaves one safe asset" 1 "$(find "$P/.agents/skilldata/auditor" -type f|wc -l|tr -d ' ')";bash "$S" "$P">"$O";eq "partial rerun completes seed" 14 "$(find "$P/.agents/skilldata/auditor" -type f|wc -l|tr -d ' ')"
...
WD="$W/.agents/skilldata/auditor/doctrine/test/workflows/audit"
...
B="$T/bound";mkdir -p "$B";if AUDITOR_SETUP_TEST_ASSET=BOOTSTRAP.md bash "$S" "$B">/dev/null 2>&1;then f=$((f+1));else p=$((p+1));fi;if AUDITOR_SETUP_TEST_DEST_REL=.agents/skilldata/inspector/doctrine/reports.md bash "$S" "$B">/dev/null 2>&1;then f=$((f+1));else p=$((p+1));fi;no "boundary no writes" "$B/.agents/skilldata"
