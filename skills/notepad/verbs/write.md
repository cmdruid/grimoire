...
1. Resolve `<root>` (SKILL.md). Locate `scripts/note-mint.sh` from
   this skill's own directory.
2. **List live notes only.** If `.records/records.sh` is
   executable: `.records/records.sh list --type notes` (no `--status`; live-set
   default). Else scan `.records/notes/*.md` and skip any file
...
3. If a **live** note already covers the fact: edit the body (one fact;
   ...
   `note-mint.sh stamp <root> <abs-path>`.
...
5. Otherwise mint:
   `note-mint.sh mint <root> "<headline>"`,
