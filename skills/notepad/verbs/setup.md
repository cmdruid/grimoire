...
1. Resolve `<root>` from the argument, else the current Git top level. Read `agent-workspace:` from
   root `AGENTS.md`, then `CLAUDE.md`, else `.spaces`; read `agent-records:` or `records-root:` only
   to detect recognized legacy templates.
2. For a standalone invocation, run package-local `scripts/notepad-setup.sh <root>`. It preflights
   ...
   `<agent-workspace>/notepad/templates/notes.md` when absent, and makes one pathspec-scoped commit
