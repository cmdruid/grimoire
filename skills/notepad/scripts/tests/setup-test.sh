...
OUT="$(bash "$SETUP" "$ROOT")"
DEST="$ROOT/.agents/skilldata/notepad/templates/notes.md"
ok test -f "$DEST"
has "created report" 'created=.agents/skilldata/notepad/templates/notes.md' "$OUT"
has "commit report" 'committed=.agents/skilldata/notepad/templates/notes.md' "$OUT"
eq "standalone commit exact path" '.agents/skilldata/notepad/templates/notes.md' "$(git -C "$ROOT" show --pretty='' --name-only HEAD | sed '/^$/d')"
...
eq "write-only leaves one owner path" '?? .agents/skilldata/notepad/templates/notes.md' "$(git -C "$SWEEP" status --short --untracked-files=all)"
...
absent "legacy not adopted" "$LEG/.agents/skilldata/notepad/templates/notes.md"
...
RACE="$TMP/recheck"; mkdir -p "$RACE/.agents/skilldata/notepad" "$RACE/elsewhere"
...
if NOTEPAD_SETUP_TEST_DEST_REL='.agents/skilldata/delegate/templates/notes.md' bash "$SETUP" --write-only "$BOUND" >/dev/null 2>&1; then echo "FAIL: sibling destination accepted" >&2; fail=$((fail + 1)); else pass=$((pass + 1)); fi
absent "boundary injections create no skilldata" "$BOUND/.agents/skilldata"
