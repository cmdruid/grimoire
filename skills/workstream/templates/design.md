...
- **Ground before designing.** Read the project vision doc + the core design principles doc and skim
  ...
  `<project: vision doc, design principles doc, and key invariants -- see host AGENTS.md + <agent-workspace>/doctrine/core/INVARIANTS.md>`
...
- **The project keystone is a design tool, not just a constraint.** The foundational architecture
  ...
  of stored?" early.
  `<project: keystone invariant -- see host AGENTS.md + <agent-workspace>/doctrine/core/INVARIANTS.md>`
...
- `<project: additional hard-won design lessons -- see the host's <agent-workspace>/doctrine/core/INVARIANTS.md>`
...
- Artifacts: the design spec (with frontmatter) in `<project: design specs location -- e.g. .records/specs/>`;
