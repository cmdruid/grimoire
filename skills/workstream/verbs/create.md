...
>   to "also" run the stream here. Instead **capture** the work (the host's bug-filing lane, or `/backlog task`) or
...
6. **Hand-off instantiation** *(the template-mode part is re-applied in place by `recycle.md` — keep
   ...
   - **Build the Cheat sheet** (the template's `## Cheat sheet` section) — a durable orientation map so
     ...
     `/architect brainstorm`/`spec` would otherwise re-derive into context that each reset destroys).
