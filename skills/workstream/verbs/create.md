...
3. **Source resolution** *(referenced by `recycle.md` — keep this label)*: resolve the second
   ...
   - **any other existing file or doc-section** (resolves to a file, a path ending `.md`, or a
     ...
     standalone `dev/plans/` plan AND a section of an ongoing roadmap (a stream/track heading whose
...
5. **Seed the plan on the branch** (plan-bound, *new untracked plan file* only; `create` makes no root
   ...
   into the worktree under `dev/plans/` (`mv <plan-path> <root>/.workstreams/<stream>/dev/plans/<basename>`
   ...
   `<target>` at first ship): `git -C <worktree> add dev/plans/<basename> && git -C <worktree> commit -m "Seed stream/<stream>: plan" -- dev/plans/<basename>`.
