...
Then invoke `workstream.sh ROOT sync STREAM`. The helper validates the registered worktree and does
nothing when the target is already an ancestor. Otherwise it rebases the stream branch onto the
recorded target without changing the target or primary checkout.
