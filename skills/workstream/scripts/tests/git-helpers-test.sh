...
mkdir -p "$WT/.records/plans"
printf '# next\n' > "$WT/.records/plans/next.md"
"$FACTS" stream-state "$WT" stream/demo main > "$OUT"
expect_eq "plan draft is classified" ".records/plans/next.md" "$(fact drafted_next_plan "$OUT")"
expect_eq "plan draft is not real WIP" "false" "$(fact wip_tracked "$OUT")"
printf 'scratch\n' > "$WT/scratch.txt"
"$FACTS" stream-state "$WT" stream/demo main > "$OUT"
expect_eq "other dirt is real WIP" "true" "$(fact wip_tracked "$OUT")"
rm "$WT/scratch.txt" "$WT/.records/plans/next.md"
rmdir "$WT/.records/plans" "$WT/.records"
