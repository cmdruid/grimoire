#!/usr/bin/env bash
# Setup and naked repair own exactly the four tracked control artifacts.
set -u
...
expect_eq 'exact four control files tracked' 4 "$(git -C "$ROOT" ls-files .streams | wc -l | tr -d ' ')"
for path in .gitignore CONFIG.md README.md workstream.sh; do if git -C "$ROOT" ls-files --error-unmatch ".streams/$path" >/dev/null 2>&1; then pass=$((pass + 1)); else fail=$((fail + 1)); fi; done
expect_eq 'setup does not create history.tsv' 0 "$([ -e "$ROOT/.streams/history.tsv" ] && echo 1 || echo 0)"
...
printf 'stream\tsequence\trecorded_at\ttarget\tunit\tcommits\tsummary\n' >"$ROOT/.streams/history.tsv"
git -C "$ROOT" add .streams/history.tsv
git -C "$ROOT" commit -qm 'incumbent history'
"$HELPER" "$ROOT" repair >"$OUT" 2>"$ERR"
expect 'repair ignores incumbent history' 'status=current' "$OUT"
expect_eq 'incumbent history remains' 1 "$([ -f "$ROOT/.streams/history.tsv" ] && echo 1 || echo 0)"
expect_eq 'repair does not stage history' 1 "$(git -C "$ROOT" diff --quiet -- .streams/history.tsv && echo 1 || echo 0)"
