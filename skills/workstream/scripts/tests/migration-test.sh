...
"$HELPER" "$ROOT2" migrate inventory >"$OUT"
...
"$HELPER" "$ROOT4" migrate inventory >"$OUT"
...
if "$HELPER" "$ROOT4" migrate apply >"$OUT" 2>"$ERR"; then fail=$((fail + 1)); else pass=$((pass + 1)); fi
...
"$HELPER" "$ROOT4" migrate inventory >"$OUT"; "$HELPER" "$ROOT4" migrate apply >"$OUT"
...
"$HELPER" "$ROOT5" migrate inventory >"$OUT"
if WORKSTREAM_TEST_AFTER_MIGRATION_TRACKER="$TMP/interrupt.sh" "$HELPER" "$ROOT5" migrate apply >"$OUT" 2>"$ERR"; then fail=$((fail + 1)); else pass=$((pass + 1)); fi
...
"$HELPER" "$ROOT5" migrate apply >"$OUT"
