...
usage: workstream-git.sh <subcommand> [args...]
  ...
  gate-facts       <worktree> <branch> <target>          markdown gate truth table
...
cmd_stream_state() {
  ...
  drafts="$(printf '%s\n' "$porcelain" | sed -n 's#^?? \(dev/plans/.*\.md\)$#\1#p' | grep -v '/archive/' | paste -sd, - || true)"
  [ -z "$drafts" ] && drafts="none"
  # Real WIP = any porcelain line that is NOT an untracked TOP-LEVEL dev/plans
  # draft (the same set `drafts` reports). An untracked file under
  # dev/plans/archive/ (or deeper) is real WIP, not a draft -- it must surface in
  # wip_tracked rather than as dirt no fact explains.
  local wip
  wip="$(printf '%s\n' "$porcelain" | grep -v '^$' | grep -vE '^\?\? dev/plans/[^/]+\.md$' || true)"
...
cmd_gate_facts() {
  [ "$#" -eq 3 ] || { echo "usage: workstream-git.sh gate-facts <worktree> <branch> <target>" >&2; exit 2; }
  local wt="$1" branch="$2" target="$3"
  validate_ref "$branch"; validate_ref "$target"

  local base own_files inc_files own_class inc_class
  ...
  # incoming diff: BASE..<target> == what <target> gained since the branch's base.
  inc_files="$(git -C "$wt" diff --name-only "$base..$target")"
...
cmd_land_readiness() {
  ...
  local root_branch root_porcelain behind ahead
