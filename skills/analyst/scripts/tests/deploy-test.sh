...
expect_eq "first deploy: five catalog kinds plus reports.md" "6" \
  "$(find "$FIX/.dev/analyst/templates" -name '*.md' | wc -l | tr -d ' ')"

# --- customization survives a re-run -----------------------------------------

MARKER="PROJECT CUSTOMIZATION — must survive every redeploy"
echo "$MARKER" >> "$FIX/.dev/analyst/templates/briefing.md"
...
expect "re-run: marker survived"            "$MARKER" \
  "$FIX/.dev/analyst/templates/briefing.md"
...
if grep -qF -- "$MARKER" "$FIX/.dev/analyst/templates/briefing.md"; then
...
expect "re-run: host-added template untouched" "template: house-style" \
  "$FIX/.dev/analyst/templates/house-style.md"
...
expect_eq "absent: creates only owner template tree" "6" \
  "$(find "$BARE/.dev/analyst/templates" -name '*.md' | wc -l | tr -d ' ')"
...
ln -s "$UNSAFE/elsewhere" "$UNSAFE/.dev"
