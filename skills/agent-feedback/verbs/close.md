...
`declined` and `stale`. It must be a privacy-safe commit, repository-relative path, URL, or another
feedback ID—never an absolute local path. Invoke package-local `scripts/feedback.sh close` exactly
