...
. "$HERE/lib.sh"
T="$(mktemp -d "${TMPDIR:-/tmp}/agent-feedback-boundary.XXXXXX")"
trap 'rm -rf "$T"' EXIT
H="$T/home"; new_home "$H"
...
NETWORK_CANARY="$T/network" PATH="$T/fake-bin:$PATH" HOME="$H" "$PROVIDER" close \
  --id "$capture_id" --as declined --reason 'The observation will not be acted on.' >/dev/null
...
[ ! -e "$H/.agents/skilldata/FEEDBACK.md" ] && pass || fail 'legacy migrated into skilldata'
