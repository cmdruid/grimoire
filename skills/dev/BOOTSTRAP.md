...
<repo root>/
  ...
  dev/
    ...
    docs/
      ...
      TAXONOMY.md         -- the artifact types, their frontmatter schema, and the search recipes
...
4. **Validate store-dir frontmatter.** Every file in an artifact-instance store dir (`dev/plans/`,
   ...
   `YYYY-MM-DD` (schema: `docs/TAXONOMY.md`). Catches records that would silently escape the
...
store-dir check, §11.4, rejects an instance without it; schema in `docs/TAXONOMY.md`). Do **not**
