# `/dev feedback` — capture a qualitative / directional note
...
This is the quick, one-shot path to the feedback log (the new sibling of `/dev backlog`); the full
end-of-work sweep is `/dev debrief`.
...
(feature → `/dev backlog`, defect → `/dev bug`, dev-friction → `/dev issue`).
...
- The user says: "/dev feedback", "log this feedback", "note how that felt", "capture this observation".

**Do NOT use** for anything actionable — a feature wish (`/dev backlog`), a defect (`/dev bug`),
dev-friction (`/dev issue`) — or for the end-of-work multi-tracker sweep (`/dev debrief`). If a note
...
The **feedback audit** (`/dev upkeep`'s `feedback` pass) periodically *drains* this file — routing
...
5. **Commit (standalone only).** Invoked **standalone**, scoped-commit via
   ...
   doc-linter. Invoked **inside `/dev debrief`/`upkeep`**, do **not** commit — only write; the sweep
   makes the single atomic commit.
6. **Report** the entry title and the path.

## Relationship to neighboring verbs

- **`/dev debrief`** — the completion sweep across *all* trackers; it routes the qualitative share
  here. `feedback` is the anytime, FEEDBACK-only, direct add.
- **`/dev upkeep`** (its `feedback` pass) drains `FEEDBACK.md` — routing actionable items to their
  real home, clearing absorbed ones. Capture is this verb's job; draining is upkeep's.
- **`/dev backlog`**, **`/dev bug`**, **`/dev issue`** — the actionable homes; this verb is the
