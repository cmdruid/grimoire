# `/dev bug` — file an observed defect
...
in `dev/docs/DEVELOPMENT.md` → *Capture follow-ups* (feature → `/dev backlog`, dev-friction →
`/dev issue`, qualitative → `/dev feedback`).
...
- The user says: "/dev bug", "file a bug", "this is broken — here's the repro", "log this defect",
  "report this crash".

**Do NOT use** for a feature wish (`/dev backlog`), tooling friction (`/dev issue`), or a qualitative
note (`/dev feedback`). And if it turns out to be **working-as-coded but surprising**, it's not a
bug — promote it to `dev/docs/GOTCHAS.md` and file no report. For the end-of-work *sweep* that routes
many surfaced items at once (including any defects), that's `/dev debrief`; this verb is the
...
6. **Commit (standalone only).** Invoked **standalone**, scoped-commit the report + its linking item
   ...
   the host's doc-linter. Invoked **inside `/dev debrief`**, do **not**
...
- **`/dev debrief`** routes *all* byproducts of a finished body of work, defects included, in one
