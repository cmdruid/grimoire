# `/dev debrief` — sweep a finished body of work, route every byproduct
...
verbs: `/dev backlog` for the `BACKLOG.md` bucket, `/dev issue` for `ISSUES.md`, `/dev feedback` for
...
- A **spike** wraps up, or any **context-rich session** ends — before the context is reset or
  handed off.
- The user asks to capture/log what came up: "/dev debrief", "capture what surfaced", "log the issues
...
`dev/done/` task record, owned by the `/workstream` land step / `/dev upkeep`. `debrief` captures the
...
format authority is its capture verb (`/dev backlog | bug | issue | feedback`). Two homes are
...
5. **Route each bucket** (each capture verb invoked here only *writes* — the single commit is step 8):
   - **BACKLOG** → apply the `/dev backlog` capture convention (group, dedupe, item = concrete
   ...
   - **ISSUES** → the `/dev issue` convention: an impact-ranked entry (what happened · impact ·
     suggested fix), continuing the file's numbering scheme.
   - **FEEDBACK** → the `/dev feedback` convention: a dated entry (`templates/feedback.md`), noting
...
7. **Verify the shipped-record exists.** Confirm the completed work has its `dev/done/` record (or
   that the `/workstream` land step / `/dev upkeep` will write it) — `debrief-scan`'s `recent_done:`
...
- **`/dev backlog`** owns the `BACKLOG.md` bucket (capture / groom); **`/dev issue`** and
  **`/dev feedback`** own `ISSUES.md` / `FEEDBACK.md`. `debrief` routes each share through that verb's
  convention; for a single-tracker add, use the verb directly. (The `dev/done/` shipped-record is
  written by the land step / `/dev upkeep`, not the capture verbs.)
...
- **`/workstream`** drives a stream in a worktree. Run `/dev debrief` when a feature — or a phase of
