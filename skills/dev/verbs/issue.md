# `/dev issue` — capture dev-experience friction
...
quick, one-shot path to the friction log (the new sibling of `/dev bug`); the full end-of-work sweep
is `/dev debrief`.
...
(defect → `/dev bug`, feature → `/dev backlog`, qualitative → `/dev feedback`).
...
- The user says: "/dev issue", "log this friction", "the tooling got in my way", "this workflow is
  painful — note it".

**Do NOT use** for a product defect (`/dev bug`), a feature wish (`/dev backlog`), or a purely
qualitative note (`/dev feedback`). A durable *gotcha* discovered here should **also** be added to
...
6. **Commit (standalone only).** Invoked **standalone**, scoped-commit via
   ...
   touched it), then run the host doc-linter. Invoked **inside `/dev debrief`/`upkeep`**, do **not**
...
- **`/dev debrief`** routes *all* byproducts of a finished body of work, friction included, in one
