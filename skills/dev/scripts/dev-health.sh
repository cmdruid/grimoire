...
#
# Read-only state analysis for a project's dev/ docs-system, for the /dev verbs
# (upkeep, debrief, backlog groom). Each subcommand emits compact `key=value`
# facts + evidence so the agent spends turns DECIDING (is this entry really
# done? is this drift real?), not scanning ten files to find the candidates.
#
...
# code `file:line` references in tracker prose, spine coverage, and the
# working-tree byproduct signals a sweep consumes.
#
# Portable over the standardized dev/ layout (`/dev init` creates it) and
