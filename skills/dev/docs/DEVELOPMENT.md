...
distinct from `BACKLOG.md` ("build X") and `ISSUES.md` ("dev-friction got in my way").
...
## Capture follow-ups -- the four trackers

A change always surfaces more than it fixes. Each follow-up has **exactly one home**, and each home
has a *drain* so it can't become a graveyard. This is the canonical taxonomy -- other docs point
here rather than re-listing it.

| Tracker | Captures | Drain |
|---|---|---|
| `BACKLOG.md` | product/feature work ("build X") | ships -> remove it (plain bullet, no checkbox), record in `dev/done/` (drain: the periodic maintenance pass -- `/dev upkeep` if available) |
| `bugs/` | defect reports -- a *store*, one file each (see *When it's a bug*) | fixed -> note the commit, `git mv` to `bugs/archive/`; tracked from a linked `BACKLOG` item |
| `ISSUES.md` | dev-experience friction ("the tooling got in my way") | resolved/documented -> record in `dev/done/`, remove |
| `FEEDBACK.md` | qualitative observations, directional ideas | periodically routed to its real home (a backlog item / bug / issue), or acted on |
...
trackers is the maintenance engine's job (`MAINTENANCE.md`).
...
everything it surfaced -- is what **`/dev debrief`** automates (if available). See `PLANNING.md` ->
