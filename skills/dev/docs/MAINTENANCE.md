# Maintenance -- keeping the dev/ system healthy

The `dev/` system's living docs **drift** as the code and filesystem change, and **accumulate** as
...
**If the host has a `/dev` skill**, its `upkeep` verb drives this whole pass (`/dev upkeep <area>` scopes to one
of spine | memory | backlog | issues | feedback | prune); otherwise run it by hand from this doc.
**If the host has a `/dev backlog` skill**, `/dev backlog groom` automates the BACKLOG sharpen/reorder
within it; removal of shipped items is part of the backlog audit (no separate prune step).
...
skill-default output paths in source, and backtick'd `dev/<path>` file refs in the stable
...
harness touches it. The contract keeps it honest as `dev/` grows.
...
- **Change `dev/` -> reflect it in `AGENTS.md` in the same commit.** A tracker, a how-to, a tool, or
...
- **Tier the pointers.** Name directly only what an agent reaches for first (`DEVELOPMENT.md`,
  `dev/MEMORY.md`, the bug docs, the trackers, the core commands); everything else hangs off an
  index (`WORKFLOWS.md` for how-tos, `dev/README.md` for the rest). When unsure, default to the
...
| **rename / move / remove** an artifact | repoint (or drop) every `AGENTS.md` pointer, and fix `dev/README.md` too |

## Audits -- catch drift

Audits keep the **dev/ docs** honest. (To audit the **project code** quality, see whatever
...
The spine -- `AGENTS.md`, `dev/README.md`, and any `README.md` files for major subdirs (plus the
root project doc and `dev/MEMORY.md`) -- is the onboarding path; it drifts whenever the filesystem
changes. If the project's operations live as skills (e.g. a `/dev` bundle), the spine audit should
also check that the skill bundles' blueprint still matches the system's structure (there is no
synced mirror to track -- the host repo's own `dev/` is the live example).
...
4. **Consistency** -- no spine doc contradicts another or the code; conventions match across
   `AGENTS.md` / `dev/README.md` / `dev/MEMORY.md`.
...
### The memory (`dev/MEMORY.md`) -- highest stakes
...
### The backlog (`dev/BACKLOG.md`)
A deep pass over **every** item: **done?** (cross-ref `git log` -> **remove** it; the commit + any
`dev/done/` stream digest is the record) · **still relevant?** (obsolete/superseded -> remove,
rationale to `done/`) · **accurate?** (repoint stale `file:line`) · **right scope?** (split or
merge) · **right classification?** (effort tag, group, order). If available, `/dev backlog groom`
...
### The issues log (`dev/ISSUES.md`)
Per entry: **resolved?** (fixed in code *or* now documented in a gotchas doc / `AGENTS.md` ->
prune to `dev/done/`) · **still a real constraint?** · **fix superseded?** (rewrite to current
...
is the source of truth; `dev/done/` is the human-readable index into it.**

- **`dev/plans/`** -- a one-shot plan or spike *ships* -> `git mv` to `plans/archive/`. Roadmaps and
  ongoing design docs stay live until the track ships.
- **`dev/adr/`** -- an ADR must stand on its own: decision rationale lives *in* the ADR, which cites
  the `dev/ROADMAP.md` track (or another ADR), **never** a plan path -- a durable doc must not depend
...
- **`dev/reports/`** -- conclusions acted on or superseded -> `reports/archive/`.
- **`dev/bugs/`** -- fixed -> note the commit in the report, `git mv` to `bugs/archive/`.
- **`dev/notes/`** -- a note is subordinate to the entry that links it (a `MEMORY`/tracker/report
...
- **The four trackers** drain per their taxonomy (`DEVELOPMENT.md` -> *Capture follow-ups*):
  `BACKLOG`/`ISSUES` items to a dated `dev/done/<YYYY-MM-DD>-<slug>.md`, `bugs/` to its archive.
  (No checkbox/prune step -- an item is removed when its work ships; the audit drains the stragglers.)
- **`dev/MEMORY.md`** -- prune a durable fact only when it goes obsolete (a reversed convention, a
