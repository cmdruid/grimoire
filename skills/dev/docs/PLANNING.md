...
Most link to a template in `dev/templates/`. Copy it, or have your planning skill produce its shape
(see *Working with your planning skills*). All planning docs live in `dev/plans/`; ADRs in `dev/adr/`.
Every artifact instance carries the uniform frontmatter block -- types, per-type status sets, and
search recipes are defined once in `dev/docs/TAXONOMY.md` (don't restate them here).
...
- **Roadmap** (`templates/roadmap.md`) -- a track's spec: the phase sequence, shared foundations,
  ...
  live in `dev/plans/` until it ships. The roadmap settles the design for *all* phases -- a phase
...
- **ADR** (`templates/adr.md`) -- a Nygard record of a cross-cutting architecture decision:
  ...
  `dev/adr/NNNN-<slug>.md` and stays live after the feature ships. (`WORKTREES.md` covers where
...
- **Don't double-plan.** Run your skill, then map its output onto the repo convention -- don't
  ...
  extra busywork.
- **Land in `dev/plans/`, never a skill-default path.** Some skills default to their own output
  dirs; this system requires `dev/plans/` (and `dev/adr/` for ADRs). Redirect the output there.
...
**Definition of done** -- run **`/dev debrief`** (if available) before the work is considered
finished, while the session still holds everything it surfaced. It sweeps the completed work and
routes each byproduct to its one home: follow-ups -> `BACKLOG.md`, dev-friction -> `ISSUES.md`,
a defect -> a linked report in `bugs/`, qualitative -> `FEEDBACK.md`, a load-bearing invariant ->
`MEMORY.md`, with any long-form context spilled to a linked `dev/notes/` file (the canonical
taxonomy is `DEVELOPMENT.md` -> *Capture follow-ups*).

This trigger is the work being **done**, not *landing* it: run it at completion -- before you
commit / merge / ship -- never as a step of the transport. If no `/dev debrief` skill is available,
...
meets its done-when run **`/dev debrief`** (the phase is finished -- capture what it surfaced while
...
the finished phase's plan to `plans/archive/`, and write a dated `dev/done/<YYYY-MM-DD>-<slug>.md`
