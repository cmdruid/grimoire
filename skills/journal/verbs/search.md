...
1. Resolve the project root, agent workspace, and agent-records home (SKILL.md discipline).
2. Staged `<agent-workspace>/journal/scripts/records.sh` missing **or not
...
4. Prefix every invocation with `records.sh --root <root> --records-root
   <records-root-relative>`, then parse the query into optional `list`-shaped filters (`--type` /
   ...
   - No body pattern (filters only, or a metadata-only ask) →
     `records.sh --root <root> --records-root <records-root-relative> list` with those filters.
   - Body pattern → `records.sh --root <root> --records-root <records-root-relative> grep` with those filters.
...
- Hits printed or shown from `records.sh --root <root> --records-root <records-root-relative> list` or `grep`.
