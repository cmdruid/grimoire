...
proj="$TMP/proj"
mkdir -p "$proj"
"$SKILL/scripts/standup.sh" "$proj" --workspace .dev --records-root .records >/dev/null
RS="$proj/.dev/journal/scripts/records.sh"
rs() { "$RS" --root "$proj" --records-root .records "$@"; }
today="$(date +%Y-%m-%d)"

# Project templates normally live under an owner's workspace `templates/`
# kind. This fixture plants contract-shaped files under the records root on
# purpose: the record discriminator must still ignore them.
...
mkdir -p "$empty_stage"
"$SKILL/scripts/standup.sh" "$empty_stage" --workspace .dev --records-root .records >/dev/null
RSE="$empty_stage/.dev/journal/scripts/records.sh"
mkdir -p "$empty_stage/.records/notes"
printf -- '---\ndoctype: notes\nstatus: draft\ncreated: %s\nupdated: %s\ntags: []\nstage:\n---\n# Empty stage\n' \
  "$today" "$today" > "$empty_stage/.records/notes/$today-empty-stage.md"
rc=0; "$RSE" --root "$empty_stage" --records-root .records check >"$OUT" 2>"$ERR" || rc=$?
...
mig="$TMP/migrate"
mkdir -p "$mig"
"$SKILL/scripts/standup.sh" "$mig" --workspace .dev --records-root .records >/dev/null
RSM="$mig/.dev/journal/scripts/records.sh"
...
"$RSM" --root "$mig" --records-root .records migrate-status >"$OUT"
...
mkdir -p "$proj_live"
"$SKILL/scripts/standup.sh" "$proj_live" --workspace .dev --records-root .records >/dev/null
RSL="$proj_live/.dev/journal/scripts/records.sh"
mkdir -p "$proj_live/.records/notes"
printf -- '---\ndoctype: notes\nstatus: draft\ncreated: %s\nupdated: %s\ntags: []\n---\n# Live default\n' \
  "$today" "$today" > "$proj_live/.records/notes/$today-live-default.md"
"$RSL" --root "$proj_live" --records-root .records 'done' "$proj_live/.records/notes/$today-live-default.md" --as 'done' >/dev/null
"$RSL" --root "$proj_live" --records-root .records list >"$OUT"
...
proj_eq="$TMP/proj-eq"
mkdir -p "$proj_eq"
"$SKILL/scripts/standup.sh" "$proj_eq" --workspace .dev --records-root .records >/dev/null
RSEQ="$proj_eq/.dev/journal/scripts/records.sh"
...
"$RSEQ" --root "$proj_eq" --records-root .records check >"$OUT"
...
rc=0; "$RSEQ" --root "$proj_eq" --records-root .records check >"$OUT" 2>"$ERR" || rc=$?
...
mkdir -p "$proj_grep"
"$SKILL/scripts/standup.sh" "$proj_grep" --workspace .dev --records-root .records >/dev/null
RSG="$proj_grep/.dev/journal/scripts/records.sh"
mkdir -p "$proj_grep/.records/notes"
printf -- '---\ndoctype: notes\nstatus: draft\ncreated: %s\nupdated: %s\ntags: [ZXSECRET]\n---\n\n# Tagged zx\n\nNo match word in the body.\n' \
  "$today" "$today" > "$proj_grep/.records/notes/$today-tagged-zx.md"
"$RSG" --root "$proj_grep" --records-root .records grep ZXSECRET >"$OUT"
...
mkdir -p "$proj2"
"$SKILL/scripts/standup.sh" "$proj2" --workspace .dev --records-root .records >/dev/null
RS2="$proj2/.dev/journal/scripts/records.sh"
...
rc=0; "$RS2" --root "$proj2" --records-root .records check >"$OUT" 2>"$ERR" || rc=$?
