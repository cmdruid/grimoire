...
run_default() {
  "$STANDUP" setup "$1" --records-root .records --workspace-root .spaces || return $?
...
expect "README shows resolved root" "records_root='.records'" "$proj/.records/README.md"
expect "README shows quoted invocation" '"$records_root/records.sh" --root "<absolute-project-root>"' "$proj/.records/README.md"
...
expect "refresh adds current path" "records_root='.records'" "$proj/.records/README.md"
...
# A custom records root gets the complete layer.
custom="$TMP/custom"
mkdir -p "$custom"
rc=0
"$STANDUP" setup "$custom" --records-root data/records \
  --workspace-root dev >"$OUT" 2>"$ERR" || rc=$?
expect_eq "custom root rc" "0" "$rc"
"$STANDUP" finalize "$custom" --records-root data/records --workspace-root dev
[ -x "$custom/data/records/records.sh" ] && pass=$((pass + 1)) || {
...
expect "custom README shows resolved path" \
  "records_root='data/records'" \
  "$custom/data/records/README.md"

# Every accepted records root must render a runnable, safely quoted command.
spaced="$TMP/spaced"
spaced_rel='team'"'"'s [records] $pace'
mkdir -p "$spaced"
rc=0
"$STANDUP" setup "$spaced" --records-root "$spaced_rel" \
  --workspace-root 'agent workspace' >"$OUT" 2>"$ERR" || rc=$?
expect_eq "spaced root standup rc" "0" "$rc"
"$STANDUP" finalize "$spaced" --records-root "$spaced_rel" --workspace-root 'agent workspace'
snippet="$(sed -n '/^    records_root=/,/^    "\$records_root\/records.sh"/p' \
  "$spaced/$spaced_rel/README.md" | sed 's/^    //' | \
  sed "s#<absolute-project-root>#$spaced#")"
rc=0
(cd "$spaced" && sh -c "$snippet" >"$OUT" 2>"$ERR") || rc=$?
expect_eq "spaced root README command rc" "0" "$rc"
...
# If the resolved records root already makes the old and new tool paths the
# same file, setup must not delete the freshly installed engine.
coincident="$TMP/coincident"
mkdir -p "$coincident"
rc=0
"$STANDUP" setup "$coincident" --records-root .spaces/journal/scripts \
  --workspace-root .spaces >"$OUT" 2>"$ERR" || rc=$?
expect_eq "coincident tool path rc" "0" "$rc"
"$STANDUP" finalize "$coincident" --records-root .spaces/journal/scripts --workspace-root .spaces
[ -x "$coincident/.spaces/journal/scripts/records.sh" ] && pass=$((pass + 1)) || {
  echo "FAIL: coincident staged engine was deleted" >&2; fail=$((fail + 1)); }
...
for marker_case in duplicate nested reversed unmatched-end; do
  marker_root="$TMP/marker-$marker_case"
  rc=0
  "$marker_mutation_skill/scripts/standup.sh" setup "$marker_root" --records-root .records \
    --workspace-root .spaces >"$OUT" 2>"$ERR" || rc=$?
...
unsafe="$TMP/unsafe"
mkdir -p "$unsafe/real"
ln -s "$unsafe/real" "$unsafe/data"
rc=0
"$STANDUP" setup "$unsafe" --records-root data/records \
  --workspace-root dev >"$OUT" 2>"$ERR" || rc=$?
expect_eq "records symlink rejected" "2" "$rc"
[ ! -e "$unsafe/real/records" ] && pass=$((pass + 1)) || {
  echo "FAIL: records symlink target mutated" >&2; fail=$((fail + 1)); }
[ ! -e "$unsafe/data/records" ] && pass=$((pass + 1)) || {
  echo "FAIL: records root partially created" >&2; fail=$((fail + 1)); }

unsafe_workspace="$TMP/unsafe-workspace"
mkdir -p "$unsafe_workspace/real"
ln -s "$unsafe_workspace/real" "$unsafe_workspace/dev"
rc=0
"$STANDUP" setup "$unsafe_workspace" --records-root .records \
  --workspace-root dev >"$OUT" 2>"$ERR" || rc=$?
...
for bad in /abs foo/../bar .; do
  rc=0
  "$STANDUP" setup "$proj" --records-root "$bad" \
    --workspace-root .spaces >"$OUT" 2>"$ERR" || rc=$?
  expect_eq "bad records root $bad" "1" "$rc"
  rc=0
  "$STANDUP" setup "$proj" --records-root .records \
    --workspace-root "$bad" >"$OUT" 2>"$ERR" || rc=$?
  expect_eq "bad workspace root $bad" "1" "$rc"
done
