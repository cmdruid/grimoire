...
mkdir -p "$powner"
rc=0
"$CRT" --root "$powner" --workspace .spaces --owner debugger --stem diagnostics \
  >"$OUT" 2>"$ERR" || rc=$?
expect_eq "owner-first create rc" "0" "$rc"
[ -f "$powner/.spaces/debugger/flows/diagnostics.md" ] && pass=$((pass + 1)) || {
  echo "FAIL: owner-first stub missing" >&2; fail=$((fail + 1)); }
[ ! -e "$powner/.spaces/shopbook/flows" ] && pass=$((pass + 1)) || {
  echo "FAIL: unrelated owner namespace created" >&2; fail=$((fail + 1)); }
retired_flows="$powner/.spaces/"flows
...
p17="$TMP/p17"
mkdir -p "$p17"
rc=0; "$CRT" --root "$p17" --workspace .spaces --owner shopbook --stem publish-release >"$OUT" 2>"$ERR" || rc=$?
expect_eq "17 create rc" "0" "$rc"
expect_eq "17 created true" "true" "$(fact created "$OUT")"
expect_eq "17 reason ok" "ok" "$(fact reason "$OUT")"
stub="$p17/.spaces/shopbook/flows/publish-release.md"
[ -f "$stub" ] && pass=$((pass + 1)) \
...
[ ! -e "$p17/.spaces/shopbook/doctrine" ] && pass=$((pass + 1)) \
...
rc=0; "$CRT" --root "$p17" --workspace .spaces --owner shopbook --stem Bug >"$OUT" 2>"$ERR" || rc=$?
expect_eq "17 Bug rc" "1" "$rc"
expect_eq "17 Bug reason" "bad-stem" "$(fact reason "$OUT")"
[ ! -e "$p17/.spaces/shopbook/flows/Bug.md" ] && pass=$((pass + 1)) \
  || { echo "FAIL: 17 Bug.md was written" >&2; fail=$((fail + 1)); }

rc=0; "$CRT" --root "$p17" --workspace .spaces --owner shopbook --stem '../x' >"$OUT" 2>"$ERR" || rc=$?
...
rc=0; "$CRT" --root "$p17" --workspace .spaces --owner flows --stem bad-owner >"$OUT" 2>"$ERR" || rc=$?
...
# Default .spaces absent → mkdir .spaces/shopbook/flows/ only.
pdev="$TMP/pdev"
mkdir -p "$pdev"
rc=0; "$CRT" --root "$pdev" --workspace .spaces --owner shopbook --stem boot >"$OUT" 2>"$ERR" || rc=$?
expect_eq "17 .spaces mkdir rc" "0" "$rc"
[ -f "$pdev/.spaces/shopbook/flows/boot.md" ] && pass=$((pass + 1)) \
  || { echo "FAIL: 17 .spaces/shopbook/flows/boot.md missing" >&2; fail=$((fail + 1)); }
[ ! -e "$pdev/.spaces/shopbook/doctrine" ] && pass=$((pass + 1)) \
...
printf 'ORIGINAL\n' > "$stub"
rc=0; bash "$TMP/crt-no-inc.sh" --root "$p17" --workspace .spaces --owner shopbook --stem publish-release >"$OUT" 2>"$ERR" || rc=$?
...
p19="$TMP/p19"
mkdir -p "$p19/.spaces"
rc=0; "$CRT" --root "$p19" --workspace .spaces --owner shopbook --stem cut-release \
  --title 'Cut: "v1" release' >"$OUT" 2>"$ERR" || rc=$?
expect_eq "19 quoted title rc" "0" "$rc"
stub19="$p19/.spaces/shopbook/flows/cut-release.md"
...
mkdir -p "$p19b/.spaces"
nl_title=$(printf 'a\nb')
rc=0; "$CRT" --root "$p19b" --workspace .spaces --owner shopbook --stem bad-nl --title "$nl_title" \
  >"$OUT" 2>"$ERR" || rc=$?
expect_eq "19 newline rc" "1" "$rc"
expect_eq "19 newline reason" "bad-value" "$(fact reason "$OUT")"
[ ! -e "$p19b/.spaces/shopbook/flows/bad-nl.md" ] && pass=$((pass + 1)) \
  || { echo "FAIL: 19 newline minted a file" >&2; fail=$((fail + 1)); }

rc=0; "$CRT" --root "$p19b" --workspace .spaces --owner shopbook --stem bad-fence --title 'a --- b' \
  >"$OUT" 2>"$ERR" || rc=$?
expect_eq "19 --- rc" "1" "$rc"
expect_eq "19 --- reason" "bad-value" "$(fact reason "$OUT")"
[ ! -e "$p19b/.spaces/shopbook/flows/bad-fence.md" ] && pass=$((pass + 1)) \
  || { echo "FAIL: 19 --- minted a file" >&2; fail=$((fail + 1)); }

rc=0; "$CRT" --root "$p19b" --workspace .spaces --owner shopbook --stem bad-ws --title '   ' \
  >"$OUT" 2>"$ERR" || rc=$?
expect_eq "19 whitespace rc" "1" "$rc"
expect_eq "19 whitespace reason" "bad-value" "$(fact reason "$OUT")"

rc=0; "$CRT" --root "$p19b" --workspace .spaces --owner shopbook --stem bad-empty --title '' \
  >"$OUT" 2>"$ERR" || rc=$?
