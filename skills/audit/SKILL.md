---
name: audit
description: Deploy and operate a code-quality audit framework via a project's dev/audit/ system. Calibrate against the host's dev/audit/GUIDE.md, scope by risk-weight, run metrics.sh, score targets against the rules/ dimensions, log raw output, record findings, and drain actionable findings to the host's trackers. Use when the user runs `/audit`, asks to audit/quality-check the codebase or a module, or to stand up the audit framework on a project that lacks one. `/audit deploy` bootstraps dev/audit/ from the bundled BOOTSTRAP.md; `/audit metrics` runs metrics.sh; `/audit check` runs the invariant gate. This audits PROJECT CODE -- distinct from any dev/ docs-system maintenance sweep.
---

# Audit skill

Drive a **code-quality audit** over a project's own code using a `dev/audit/` system.
This skill is a **thin driver**: the methodology, rubric, scoring rules, and severity
model live in the host's `dev/audit/GUIDE.md` (+ the per-dimension `rules/` files) and
are the single source of truth -- **re-read `GUIDE.md` each pass**; this skill
orchestrates the loop, it does not restate the rubric.

If the host project has **no `dev/audit/` yet**, deploy it first (see *Deploy mode*),
...
`dev/audit/`**, as it fills in, as the live example.

**Scope boundary:** `/audit` audits **project code** (quality + the host's sacred
invariants). It is **not** a docs-system maintenance sweep. Different domain.

## Modes (selected by argument)

- **`deploy`** (alias `init`) -- stand up `dev/audit/` on a project that lacks one.
- **(no arg) -- a pass.** Run the audit loop, scoped to a risk-weighted target (or all).
- **`metrics`** -- run the host's `dev/audit/metrics.sh`: print the report, append the
  trend row to `dev/audit/logs/metrics.csv`. No scoring.
- **`check`** -- run `dev/audit/metrics.sh --check`: the invariant gate (a non-zero
  count of the host's native-invariant smell is a P0 and fails).
- **`<target>`** -- a path scopes a pass to that target at its depth.

## Deploy mode

When the host has no `dev/audit/`, follow the bundled `BOOTSTRAP.md`:
...
2. **Author the rubric** -- copy the bundled generic `rules/` into the host's
   `dev/audit/rules/`, fill the `<language>` greps and `How to quantify` recipes, and
...
Follow the host's `dev/audit/GUIDE.md` -> *Process*; in brief:
...
3. **Quantify.** Run `dev/audit/metrics.sh` for reproducible counts *before* scoring;
...
5. **Log.** Append raw output + per-run notes under `dev/audit/logs/` (dated, one
   file/pass).
...
When `FINDINGS.md` grows large, sweep resolved entries into `dev/audit/history/`.
...
`BOOTSTRAP.md` is **canonical here** -- edit it in place (there is no `dev/` copy). The
generic `rules/` are an authored distillation -- update them by hand when a dimension's
*method* changes. There is no mirror to re-sync; this repo's own `dev/audit/` is the
live worked example.
...
`Graduated:` backfilled. For a **deploy**: a `dev/audit/` that passes the host's gate,
