...
rc=0
for t in seed-test.sh face-test.sh migrate-scan-test.sh lint-exemption-test.sh setup-journal-test.sh hooks-fill-test.sh flows-door-test.sh flows-copy-test.sh; do
  echo "== $t"
  bash "$DIR/$t" || rc=1
done
