#!/usr/bin/env bash
# flows-door-test.sh — red-proofs 5, 8–11. Patient-zero: mktemp only.
set -u
...
# Disable path substitution: hardcode .dev in a copy; check stays 1 after apply.
awk '
...
rc=0; "$DOOR" apply --root "$p9" --workspace .dev >"$OUT" 2>"$ERR" || rc=$?
expect_eq "9 apply rc (no-op stop)" "0" "$rc"
expect_eq "9 apply writes nothing" "$sum9" "$(hash_of "$p9/AGENTS.md")"
rc=0; "$DOOR" check --root "$p9" --workspace .dev >"$OUT" 2>"$ERR" || rc=$?
...
p10="$TMP/p10"
mkdir -p "$p10"
rc=0; "$DOOR" apply --root "$p10" --workspace .dev >"$OUT" 2>"$ERR" || rc=$?
...
rc=0; "$DOOR" check --root "$p10" --workspace .dev >"$OUT" 2>"$ERR" || rc=$?
...
rc=0; "$DOOR" apply --root "$p10c" --workspace .dev >"$OUT" 2>"$ERR" || rc=$?
...
rc=0; "$DOOR" check --root "$p10c" --workspace .dev >"$OUT" 2>"$ERR" || rc=$?
...
# oven bytes still there (not a rewrite of that span)
