...
MOST DURABLE   .handbook/design/VISION.md        — what the product IS (north star)
   ...
   │           .handbook/design/MAP.md           — system index + seam graph             by /clankshop design health)
...
- **REFERENCE-ARCHITECTURE (disposable).** The current shape of the implementation — explicitly
  ...
  (`/clankshop design health`) can catch drift between the pointers and the code they point at.
...
- A **clear run** (`/feature plan`+`build` executing a `/clankshop design prep` brief) removes the retired
...
(never fold clear+build into one pass); `/clankshop design`'s job ends once the prep brief exists. (Forward
reference: this is the mechanism `/clankshop design prep`, Plan B, hands to `/feature`.)

## The seam — altitude, not medium

> `/clankshop design` owns the *seed-altitude standing design*; `/feature` owns *feature-scope change +
> execution*.

The seam is **altitude**, not "docs vs. code" — `/feature design`/`plan` already produce documents
too, so docs-vs-code was never the real line. `/clankshop design` reasons about and edits the *durable
...
`/clankshop design` never writes executable code, but that is a *consequence* of authoring seed-altitude
plans, not the reason the skill exists. Don't define `/clankshop design` by what it refuses to touch —
...
> `/clankshop design brainstorm|plan` mutate **the seed itself** (the foundation you later regenerate code
> from). *Changing the foundation → `/clankshop design`. Building on it → `/feature`.*

`/clankshop design`'s own non-seed wiring (for example, updating a host's doc tooling when a folder moves) is
a **handoff to `/clankshop route` or `/feature`**, not done by `/clankshop design` itself — that is code/operational
...
adopting the rebuild workflow on any real project — not a routine check `/clankshop design health` can
substitute for.
...
`check` is necessary but not sufficient (§ The one thing this script cannot tell you, `verbs/design/health.md`):
...
`/clankshop design distill` when the drift is accreted-ADR smear) vs **code drifted** (the code strayed from
...
read-test (§ Sufficiency) — and only then a fold into `.handbook/design/` via `/clankshop design seed`
