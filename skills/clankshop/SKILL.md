...
The doctrine home sits at a fixed subpath of the **agent-workspace** home — declared
`agent-workspace:` at line start in the door, else `.dev`. Resolve the home, *then* test for
the artifact; a host that wants the workspace elsewhere moves it with one declaration.
...
deployed doctrine home documents itself.
