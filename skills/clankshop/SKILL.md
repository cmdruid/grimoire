...
| verb | does | status |
|---|---|---|
| `setup` | greenfield bootstrap: facts by script, decisions by interview, project the doctrine through the facts into `AGENTS.md` + `.handbook/` + `.records/`; write the installation block, the compiled tier-0 table, the stewardship maps, each member's door registration | lands in Phase 1 of the rollout plan |
