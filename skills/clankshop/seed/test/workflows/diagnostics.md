...
walks the bug lane. This seed carries the universal discipline; per-symptom chapters accrue as
the project earns them — an investigation that surfaced a missing chapter is exactly the signal
that adds one (a test-station chore).
...
2. **Trace** — read the actual error in full, then follow the data flow backward from the
   failure to its origin. Check `core/GOTCHAS.md` with what you learn.
...
Each chapter: `## <symptom class>` — first moves, the observability tools to reach for, the
known dead ends. Seeded empty:
