...
the history ledger). Pending renames: `feature` → `blueprint`, `handoff` → `checkpoint`;
`scheduler` is a new port. The manifest tracks each rename as it lands. The `bug`/`task`
capture proxies remain optional members.
