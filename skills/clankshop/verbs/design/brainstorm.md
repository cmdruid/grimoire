# `/clankshop design brainstorm` — foundation-altitude ideation on the seed
...
verb licensed to ask that question and act on the answer. See `docs/DESIGN-DOCTRINE.md` for the
...
reshape anything below it in the durability gradient (per `docs/DESIGN-DOCTRINE.md` § The durability
...
`/clankshop design brainstorm` ends in an **edited standing spec** — a diff to a file already living under
...
4. **Surface the blast radius, but don't chase it.** A tenet or contract change usually
   ...
   sequencing that cascade is `/clankshop design plan`'s job, not this verb's. `brainstorm` changes the one
...
explicitly as *unactioned*, for the human to route to `/clankshop design plan` if they want it sequenced.
