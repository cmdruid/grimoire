# `/clankshop design reconcile` — the deep semantic seed↔code drift check
...
2. **It writes ONLY to `.records/`.** Its whole output is one drift report under `.records/reports/`.
   ...
   (`docs/DESIGN-DOCTRINE.md` § Deliverables in `.records/`). `reconcile` also **never writes executable
...
- **A named seam the code moved or dropped.** The contract names an interface/seam (seams are binding —
  `docs/DESIGN-DOCTRINE.md`) that the code has relocated to a different boundary, merged away, or deleted.
...
- **Seed stale — the code is the intended reality.** The implementation is where the design actually
  ...
  that were never folded back (an ADR-smear, `docs/DESIGN-DOCTRINE.md` § Two temporal kinds of doc), recommend
  **`/clankshop design distill`** for that system rather than a hand-patch — distill is the verb that collapses
...
- **Per-system divergences** — each divergence from Step 3 (contract clause ↔ code `src/…:NN` ↔ the
  ...
  (seed update / `/clankshop design distill` / code defect to fix / human decides).
...
go to a human editorial pass or `/clankshop design distill`, which are code defects for the caller). Make the
