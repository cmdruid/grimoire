# `/clankshop route` (route) — classify a change and dispatch it

Hat: `roles/foreman.md` — read the hat first; you operate this verb wearing that hat.

The default verb: with no argument (or a change description), `/clankshop route` is the **change router**.
...
3. **Route**, dispatching to the lane's entry point (each lane file's by-hand walk is the co-equal
   ...
   - **foundational / seed-altitude design change** (a tenet, a system contract, a seam — not a
     feature to build) → `/clankshop design brainstorm` (then `/clankshop design plan` to sequence the rollout).
   ...
   - **code-quality** → `/auditor`; **docs-quality** → `/clankshop docs`; **context snapshot** →
     `/handoff`.
4. Where a lane's skill isn't installed, follow the lane file's by-hand walk. `/clankshop route` owns the
