...
has "$REVIEW" 'on the next turn with the complete in-context findings' "failing next-turn parse missing"

transition() {
  local kind="$1" verdict="$2" answer="${3:-}"
  [ "$kind" = implementation ] && { echo verdict-only; return; }
  if [ "$verdict" = needs-rework ]; then
    case "$answer" in refine|revise|fold) echo refine-next ;; stop|"not yet") echo no-write ;; *) echo ask ;; esac
  else
    case "$answer" in yes|approved|proceed) echo publish-reviewed ;; stop|"not yet"|refine) echo no-write ;; *) echo ask ;; esac
  fi
}
eq "failing review offers next-turn refine" refine-next "$(transition document needs-rework refine)"
