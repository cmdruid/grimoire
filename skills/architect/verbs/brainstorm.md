...
`.agents/design/` says is durably true, or does it only change what gets built on top of what's already
...
`.agents/design/` (or a new `.agents/design/src/<system>.md` if the session concludes a system needs to exist that
...
1. **Name the altitude target before opening the dialogue.** One of: a `PHILOSOPHY.md` tenet
   (add, revise, or retire one), a system's `CONTRACT` tier in `.agents/design/src/<system>.md`, a seam
...
3. **Land the terminal artifact as a seed edit.** Write the change directly into the target file:
   ...
   `.agents/design/src/<system>.md`, an updated seam-graph entry in `MAP.md`, or a revised section of
