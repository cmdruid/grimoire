...
- A named path resumes a draft only when it resolves beneath the canonical
  `<agent-workspace>/architect/drafts/` home, is a regular non-symlink Markdown file, and follows the
...
5. **Stop or persist.** For an ordinary brainstorm, recap the current synthesis in conversation and
   ...
   `<agent-workspace>` through SKILL.md *Project homes*, fill the package-only `templates/draft.md` outline as one current synthesis
