...
durability gradient for what `distill` is allowed to touch (it writes `.agents/design/src/<system>.md`
...
keeping them separate from the seed (`docs/DOCTRINE.md` § Two temporal kinds of doc): zero
...
1. **Gather.** Collect every change-record touching the target system since its last
   `distilled_through_*` stamp (`.agents/design/src/<system>.md` frontmatter — `none` means everything
...
3. **Fold into the standing spec, present-tense.** Rewrite the affected sections of
   `.agents/design/src/<system>.md` as though the reconciled result were designed whole, today — using
...
4. **Promote recurring insight up the gradient — candidate signal, human decides.** If the same
   ...
   `PHILOSOPHY.md`'s own promotion rule and `docs/DOCTRINE.md`. It is not automatic: name the
...
6. **Re-stamp.** Update `.agents/design/src/<system>.md`'s frontmatter to the new baseline:
