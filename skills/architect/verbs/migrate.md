...
2. Inventory in stable path order. Own only `doctype: specs` plus tag `spec`, `doctype: adr`, an
   exact founding shape, current `doctype: spikes` records declaring `architect/spike@1`, and active
   `specs.md`/`adr.md` legacy templates. A directory selection skips proven non-owned files and
   refuses ambiguity. Current schemas are `architect/spec@1`, `architect/adr@1`,
   `architect/founding@1`, and `architect/spike@1`; any other declared schema refuses. An explicitly
   selected valid current spike is a reported no-op. A schema-less would-be spike refuses because
   Architect defines no legacy spike shape.
