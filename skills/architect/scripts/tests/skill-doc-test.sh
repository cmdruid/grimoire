...
expect_match "workspace draft is recognized" '.agents/skilldata/architect/drafts/' "$spec_text"
...
router_text="$(tr '\n' ' ' <"$SKILL/SKILL.md")"
expect_match "workspace is fixed" 'fixed `.agents/skilldata`' "$router_text"
expect_match "records are fixed" 'fixed `.agents/skilldata` and `.records`' "$router_text"
