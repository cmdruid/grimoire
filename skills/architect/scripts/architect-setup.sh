...
[ -z "${ARCHITECT_SETUP_TEST_AFTER_PREFLIGHT:-}" ]||"$ARCHITECT_SETUP_TEST_AFTER_PREFLIGHT" "$root" "$skilldata"
while IFS= read -r f;do [ -n "$f" ]||continue; src="$base/templates/$f"; rel="$skilldata/architect/templates/$f"; dst="$root/$rel"; if [ -f "$dst" ]&&[ ! -L "$dst" ];then printf 'preserved=%s\n' "$rel";continue;fi; ensure "${rel%/*}";check "${rel%/*}";[ ! -L "$dst" ]&&[ ! -e "$dst" ]||die "destination changed after preflight: $dst";cp "$src" "$dst";made+=("$rel");n=$((n+1));printf 'created=%s\n' "$rel";[ -z "${ARCHITECT_SETUP_TEST_AFTER_WRITE:-}" ]||"$ARCHITECT_SETUP_TEST_AFTER_WRITE" "$root" "$skilldata" "$rel" "$n";done <<EOF
$files
EOF
