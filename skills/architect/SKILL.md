---
name: architect
description: "The design-system engine — verbs over a project's `.agents/design/` seed: the present-tense, regenerable design that code is a build output of. `/architect init` stands up `.agents/design/` (compiles a PROJECT.md brief, or migrates existing docs). `/architect brainstorm` and `/architect plan` do seed-altitude (foundational, not feature-scope) design work. `/architect distill` collapses accreted ADRs/plans into clean present-tense specs at a milestone. `/architect check` validates seed health (spine, drift, distill-debt). `/architect prep` (Plan B) plans clearing retired code so `/feature` can rebuild. Use when the user runs `/architect ...`, sets up a design-doc system, distills ADRs into a coherent spec, or makes a foundational (not incremental) design change. Seed-altitude peer to /feature (feature-scope build engine) and /dev (operational system)."
---

# architect — the design-system engine

A skill over a project's `.agents/design/` **seed**: the clean, present-tense, regenerable source of
...
| verb | file | one-liner |
|---|---|---|
| `init` | `verbs/init.md` | compile a PROJECT.md brief (or migrate existing docs) into `.agents/design/` |
...
3. Portable methodology stays in this package; project content stays in the project's `.agents/design/`.
