...
One verb, two passes: the **mechanical** gate (`scripts/skills-lint.sh`) and the **judgment** pass
(`docs/BOUNDARY-AUDIT.md`'s workflow). Run the mechanical pass often (cheap, deterministic); run the
full boundary audit when a description/body changed or before a release — it needs a read, not just a
script run.

## When to use

- Before committing any change under a library's `skills/` tree (the mechanical pass, at minimum).
...
| lint `WARN: edge type … declared by only one skill` | an orphan type or a consumer not yet wired | expected mid-rollout; a standing single-skill type (an intra-skill produces↔consumes chain) is a legitimate, permanent WARN — don't chase it |
...
Pass 1 reports `fails=0`; every residual WARN is either fixed or a documented exception; if Pass 2 ran,
the routing-probe passed against the thinned descriptions alone and the findings + probe result are
reported.
