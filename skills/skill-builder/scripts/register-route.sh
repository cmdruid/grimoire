...
#
# DOCTRINE: this is skill-builder's REFERENCE copy of the mechanism -- every
# durable-home skill (backlog, feature, architect, auditor, foreman, ...) bundles
# its OWN copy rather than calling out to this one, because a self-contained
# skill directory carries no cross-skill script dependency (BL-6: a shared file
# would make those skills silently depend on skill-builder being installed,
# breaking "self-init, no floor"). `skill-builder new` stamps a fresh copy of
# THIS file into a new skill; `skill-builder check` diffs every deployed copy's
# FUNCTIONAL body (comments stripped -- each skill's header legitimately differs)
# against this one and WARNs on drift. Keep this file the source of truth for the
# mechanism; a format change (delimiter syntax, write-protocol rule) lands here
# first, then re-propagates via `check`'s drift report.
set -euo pipefail
