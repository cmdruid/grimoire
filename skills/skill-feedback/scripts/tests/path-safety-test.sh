...
. "$HERE/lib.sh"
T="$(mktemp -d "${TMPDIR:-/tmp}/skill-feedback-paths.XXXXXX")"
trap 'rm -rf "$T"' EXIT
...
ok test -f "$real_home/.agents/skilldata/skill-feedback/feedback.tsv"
...
for path_case in agents-symlink skilldata-symlink owner-symlink file-symlink agents-file skilldata-file owner-file file-directory lock-symlink lock-file; do
  ...
  case "$path_case" in
    ...
    skilldata-symlink)
      ...
      ;;
    owner-symlink)
      mkdir -p "$H/.agents/skilldata"; ln -s "$outside" "$H/.agents/skilldata/skill-feedback"
      ;;
    file-symlink)
      mkdir -p "$H/.agents/skilldata/skill-feedback"
