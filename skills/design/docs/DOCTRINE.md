...
| Home | `dev/` (ADRs, plans) — operational | `design/` — the seed |
...
- **`design/src/`** holds the compilable source specs, roughly 1:1 with code units. The compile
  metaphor is exact: `design/src/<system>.md` is to `src/<code>` as a source file is to its build
...
Each `design/src/<system>.md` carries two tiers with different authority:
