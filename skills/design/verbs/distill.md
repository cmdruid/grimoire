...
durability gradient for what `distill` is allowed to touch (it writes `design/src/<system>.md`
...
1. **Gather.** Collect every change-record touching the target system since its last
   `distilled_through_*` stamp (`design/src/<system>.md` frontmatter — `none` means everything
...
2. **Reconcile the net effect.** This is the step with real judgment in it, and the one most
   ...
   stay in `dev/` untouched. `distill` only changes what the *standing spec* says; it never edits
   or deletes an ADR.

3. **Fold into the standing spec, present-tense.** Rewrite the affected sections of
   `design/src/<system>.md` as though the reconciled result were designed whole, today — using
