# `/foreman init` — stand up the `.agents/dev/` system on a fresh project

Deploy a project's `.agents/dev/` development system where none exists. This verb is a **thin driver**: the
...
glue you write** (the host's `AGENTS.md`, step 8) so every companion skill and agent **reads the
...
3. **Copy the generic `docs/`** into the host's `.agents/dev/docs/` and fill the slots.
4. **Copy the generic `templates/`** into the host's `.agents/dev/templates/`, adjusting any host specifics.
5. **Write the trackers + `.agents/dev/README`** from the `BOOTSTRAP` templates (empty files with their
...
8. **Surface the operational entrypoints in the host's `AGENTS.md`** — name the **gate** command
   ...
   *generically* — "run the host's gate", "the fast doc-linter", "the host's diagnostics tooling" —
   ...
   "run the gate" has nothing to resolve to. **Also record the on-disk home root here** (the
   `.agents/dev/` — and `.agents/design/` — location) so the recorded pointer, not a hardcoded path,
   is what agents resolve (§3.1). (`.agents/dev/docs/` holds the longer-form detail; `AGENTS.md` is
