...
4. **Record the outcome on the source signal.** For each `FEEDBACK`/`ISSUES` entry you acted on,
   record the resolution and clear it (or hand it to `/backlog groom` to drain from the list) — the
...
- **`/backlog groom`** owns the tracker lists (sharpen, reorder, remove shipped, archive). `tune`
  consumes their signal; it does not curate the lists.
...
workflow / `AGENTS.md` edits (or filed back to `/backlog` when too big), each acted-on source entry
