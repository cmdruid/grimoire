...
`migrate` does the **whole job**: locate the existing dev-meta → propose a complete old→new mapping →
confirm/edit with the user → relocate with `git mv` → scaffold the gaps → write the ownership index.
...
### 3. Classify + build the mapping (fact-gathering — propose, don't decide)
...
### 4. Present + confirm (preview-first — the gate)
...
### 6. Scaffold the gaps + write the index (this is `init`'s back half)
...
- For every home that received **no** migrated content, **scaffold it by reusing `init`** (read
  ...
  the generic `docs/` copied into `.agents/foreman/docs/` with slots filled, and the trackers.
- **Write/update the ownership index** — `.agents/README.md` + `.records/README.md` mapping content →
  location → steward — over the combined result (migrated + scaffolded), and add the front-door
  pointer, exactly as `init` does.
...
- Commit the scaffold + index scoped to their paths via the pathspec-atomic helper (may be the same
  commit as step 5, or a follow-on — either way, scoped, no `Co-Authored-By`).

### 7. Report

Chat summary (not a written file unless asked):

- **What moved where** — the applied old→new table.
...
The existing dev-meta has been located, its **complete** mapping proposed and **user-approved**,
relocated with `git mv` (history preserved, no target clobbered), the gaps scaffolded via `init`'s
scaffold, and the **ownership index written + version stamped** — so the project is fully set up and
passes `/foreman check`, having absorbed its prior content.
