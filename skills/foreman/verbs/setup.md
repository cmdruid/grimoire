...
Resolve `<root>` and `.agents/skilldata`. Run package-local `scripts/foreman-door.sh check` first.
...
Setup never creates `.agents/skilldata/foreman/operations/`, doctrine, a catalog snapshot, or a
