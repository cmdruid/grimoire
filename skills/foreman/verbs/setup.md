...
Resolve `<root>` and `<agent-workspace>`. Run package-local `scripts/foreman-door.sh check` first.
...
Setup never creates `<agent-workspace>/foreman/operations/`, doctrine, a catalog snapshot, or a
