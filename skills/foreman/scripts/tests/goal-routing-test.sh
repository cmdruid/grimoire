...
"$CTX" --root "$R" --goal "$GOAL" >"$OUT"; eq "no owner" none "$(fact owner "$OUT")"; eq "missing recovery warning" unavailable "$(fact recovery "$OUT")"
printf 'Goal: %s\nNext: /foreman goal resume %s\n' "$GOAL" "$GOAL" >"$R/CHECKPOINT.md"
"$CTX" --root "$R" --goal "$GOAL" >"$OUT"; eq "root owner" checkpoint "$(fact owner "$OUT")"
rm "$R/CHECKPOINT.md"; printf 'source: %s\nNext: /foreman goal resume %s\n' "$GOAL" "$GOAL" >"$R/WORKSTREAM.md"
"$CTX" --root "$R" --goal "$GOAL" >"$OUT"; eq "stream owner" workstream "$(fact owner "$OUT")"
printf 'Goal: %s\n' "$GOAL" >"$R/CHECKPOINT.md"
if "$CTX" --root "$R" --goal "$GOAL" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
...
"$broken" --root "$R" --goal "$GOAL" >"$OUT"; lacks "disabled guard misses refusal" 'dual-runtime-state' "$OUT"

for needle in 'ordinary `/checkpoint save`' 'writes `CHECKPOINT.md`' \
...
mkdir -p "$G/.records/goals" "$G/.spaces/foreman/operations"; printf 'goal\n' >"$G/.records/goals/committed.md"; printf 'operation\n' >"$G/.spaces/foreman/operations/committed.md"
git -C "$G" add .records/goals/committed.md .spaces/foreman/operations/committed.md; git -C "$G" commit -qm seed
if git -C "$G" cat-file -e HEAD:.records/goals/committed.md && git -C "$G" cat-file -e HEAD:.spaces/foreman/operations/committed.md; then pass=$((pass+1)); else fail=$((fail+1)); fi
...
ok test ! -e "$R/.spaces/foreman/runtime"; ok test ! -e "$R/.records/runtime"
