...
if "$DOOR" apply --root "$R" --workspace .spaces >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
ok test ! -e "$R/AGENTS.md"
"$DOOR" apply --root "$R" --workspace .spaces --allow-create >"$OUT"
has "minimal route" '<!-- skill:foreman BEGIN -->' "$R/AGENTS.md"
has "identity rule" '<owner>/<stem>' "$R/AGENTS.md"
ok test ! -e "$R/.spaces"
sum="$(cksum "$R/AGENTS.md")"; "$DOOR" apply --root "$R" --workspace .spaces >"$OUT"
...
before="$(cksum "$R2/AGENTS.md")"; "$DOOR" apply --root "$R2" --workspace dev >"$OUT"
has "foreign bytes kept" KEEP "$R2/AGENTS.md"; has "resolved workspace" 'dev/<owner>/operations' "$R2/AGENTS.md"
lacks "default workspace absent" '.spaces/<owner>/operations' "$R2/AGENTS.md"
ok test "$before" != "$(cksum "$R2/AGENTS.md")"

R3="$T/malformed"; mkdir -p "$R3"; printf '%s\n' '<!-- skill:foreman BEGIN -->' broken >"$R3/AGENTS.md"
sum="$(cksum "$R3/AGENTS.md")"; if "$DOOR" apply --root "$R3" --workspace .spaces >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
