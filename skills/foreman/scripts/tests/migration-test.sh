...
has "conforming reported" 'conforming=.spaces/publisher/operations/release.md' <("$CENSUS" --root "$R" --source .spaces/publisher/operations/release.md)
...
"$WRITE" migrate-batch --root "$PART" --batch "$T/partial.tsv" >"$OUT"; eq "partial rerun remaining" 1 "$(fact writes "$OUT")"; eq "partial converged" 2 "$(find "$PART/.spaces/foreman/operations" -type f | wc -l | tr -d ' ')"
