...
OUT="$T/out"; R="$T/root"; mkdir -p "$R/source/vendor" "$R/source/dist" "$R/.agents/skilldata/publisher/operations"
...
awk '{print}' "$FIX/clean-operation.md" >"$R/.agents/skilldata/publisher/operations/release.md"
source_sum="$(cksum "$R/source/release.md")"; existing_sum="$(cksum "$R/.agents/skilldata/publisher/operations/release.md")"

"$CENSUS" --root "$R" --source source >"$OUT"
has "text admitted" 'item=source/release.md|digest=sha256:' "$OUT"
has "conforming reported" 'conforming=.agents/skilldata/publisher/operations/release.md' <("$CENSUS" --root "$R" --source .agents/skilldata/publisher/operations/release.md)
...
eq "preview write free" 0 "$(find "$R/.agents/skilldata/foreman" -type f 2>/dev/null | wc -l | tr -d ' ')"
eq "source unchanged" "$source_sum" "$(cksum "$R/source/release.md")"

mkdir "$T/outside"; printf 'outside\n' >"$T/outside/runbook.md"
if "$CENSUS" --root "$R" --source ../outside >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
has "canonical source boundary" 'reason=source-outside-root' "$OUT"
if "$CENSUS" --root "$R" --source 'source/pipe|reason=fake.md' >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
has "explicit unsafe source path" 'reason=unsafe-source-path' "$OUT"

EMPTY="$R/empty"; mkdir "$EMPTY"; "$CENSUS" --root "$R" --source empty >"$OUT"; eq "zero census" 0 "$(fact items "$OUT")"
"$CENSUS" --root "$R" --source source/release.md >"$OUT"; eq "one census" 1 "$(fact items "$OUT")"
...
if "$WRITE" migrate-batch --root "$BOUND" --batch "$T/unsafe-source.tsv" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
has "writer rejects fact delimiter source" 'reason=unsafe-source' "$OUT"; ok test ! -e "$BOUND/.agents/skilldata"
printf 'foreman/bad-row\t%s\tsource/release.md\t%s\textra\n' "$child" "$sd" >"$T/bad-row.tsv"
if "$WRITE" migrate-batch --root "$R" --batch "$T/bad-row.tsv" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
...
if "$WRITE" migrate-batch --root "$SUB" --batch "$T/subset.tsv" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
has "subset closure refusal" 'reason=incomplete-or-invalid-closure' "$OUT"; ok test ! -e "$SUB/.agents/skilldata"

"$WRITE" migrate-batch --root "$R" --batch "$batch" >"$OUT"
eq "many candidates written" 2 "$(fact writes "$OUT")"; ok test -f "$R/.agents/skilldata/foreman/operations/release-workflow.md"
eq "source still exact" "$source_sum" "$(cksum "$R/source/release.md")"; eq "foreign incumbent exact" "$existing_sum" "$(cksum "$R/.agents/skilldata/publisher/operations/release.md")"
ok test ! -e "$R/.agents/skilldata/foreman/migrations"; ok test ! -e "$R/.agents/skilldata/foreman/manifest.tsv"
"$WRITE" migrate-batch --root "$R" --batch "$batch" >"$OUT"; eq "rerun no writes" 0 "$(fact writes "$OUT")"; eq "rerun preserves two" 2 "$(fact preserved_count "$OUT")"
...
if "$WRITE" migrate-batch --root "$DRIFT" --batch "$T/drift.tsv" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
has "source drift" 'reason=source-drift' "$OUT"; ok test ! -e "$DRIFT/.agents/skilldata"
...
if FOREMAN_WRITE_TEST_AFTER_WRITE="$hook" "$WRITE" migrate-batch --root "$PART" --batch "$T/partial.tsv" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
eq "partial wrote one" 1 "$(find "$PART/.agents/skilldata/foreman/operations" -type f | wc -l | tr -d ' ')"
"$WRITE" migrate-batch --root "$PART" --batch "$T/partial.tsv" >"$OUT"; eq "partial rerun remaining" 1 "$(fact writes "$OUT")"; eq "partial converged" 2 "$(find "$PART/.agents/skilldata/foreman/operations" -type f | wc -l | tr -d ' ')"
...
if "$WRITE" migrate-batch --root "$FOREIGN" --batch "$T/foreign.tsv" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
has "foreign migration refused" 'reason=foreign-owner' "$OUT"; ok test ! -e "$FOREIGN/.agents/skilldata"
