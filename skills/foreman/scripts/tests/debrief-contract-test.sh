...
R0="$T/no-acceptance"; mkdir "$R0"; ok test ! -e "$R0/.agents/skilldata/foreman/doctrine"
...
ok test -f "$R/.agents/skilldata/foreman/operations/session-release.md"
...
ok test -f "$R/.agents/skilldata/foreman/doctrine/release-evidence.md"
ok test ! -e "$R/.agents/skilldata/foreman/debriefs"; ok test ! -e "$R/.records/debriefs"
if rg -l -F -f "$deny" "$R/.agents/skilldata" >/dev/null 2>&1; then fail=$((fail+1)); else pass=$((pass+1)); fi
...
has "disabled debrief guard plants marker" SECRET_DO_NOT_PERSIST "$R/.agents/skilldata/foreman/operations/tainted.md"
