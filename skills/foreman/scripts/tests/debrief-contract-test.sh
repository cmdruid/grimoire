...
clean="$FIX/migration/clean-operation.md"; "$WRITE" put --root "$R" --workspace .spaces --identity foreman/session-release --candidate "$clean" --deny-list "$deny" >"$OUT"
ok test -f "$R/.spaces/foreman/operations/session-release.md"
if "$WRITE" put --root "$R" --workspace .spaces --identity foreman/tainted --candidate "$FIX/migration/tainted-operation.md" --deny-list "$deny" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
...
"$WRITE" doctrine --root "$R" --workspace .spaces --stem release-evidence --candidate "$doctrine" --deny-list "$deny" >"$OUT"
...
"$broken" put --root "$R" --workspace .spaces --identity foreman/tainted --candidate "$FIX/migration/tainted-operation.md" --deny-list "$deny" >/dev/null
