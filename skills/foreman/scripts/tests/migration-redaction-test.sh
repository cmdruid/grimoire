...
ok test -f "$R/.agents/skilldata/foreman/operations/clean.md"
...
has "taint refused" 'reason=tainted-candidate' "$OUT"; ok test ! -e "$R/.agents/skilldata/foreman/operations/tainted.md"
lacks "deny bytes absent from output" 'SECRET_DO_NOT_PERSIST' "$OUT"
if rg -l 'SECRET_DO_NOT_PERSIST' "$R/.agents/skilldata" >/dev/null 2>&1; then fail=$((fail+1)); else pass=$((pass+1)); fi
...
has "disabled guard plants marker" 'SECRET_DO_NOT_PERSIST' "$R/.agents/skilldata/foreman/operations/tainted.md"
