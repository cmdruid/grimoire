...
"$WRITE" migrate-batch --root "$R" --workspace .spaces --batch "$T/clean.tsv" --deny-list "$deny" >"$OUT"
...
if "$WRITE" migrate-batch --root "$R" --workspace .spaces --batch "$T/tainted.tsv" --deny-list "$deny" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
...
"$broken" migrate-batch --root "$R" --workspace .spaces --batch "$T/tainted.tsv" --deny-list "$deny" >/dev/null
