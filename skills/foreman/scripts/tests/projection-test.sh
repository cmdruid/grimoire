...
if "$DOOR" check --root "$R" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
eq "drift detected" true "$(fact drift "$OUT")"; "$DOOR" apply --root "$R" >"$OUT"; has "drift repaired" '.spaces/<owner>/operations' "$R/AGENTS.md"; has "foreign still kept" KEEP "$R/AGENTS.md"
