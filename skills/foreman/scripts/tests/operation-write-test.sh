...
if FOREMAN_WRITE_TEST_DEST_REL='.spaces/debugger/operations/escape.md' "$WRITE" put --root "$R" --identity foreman/escape --candidate "$C" >"$OUT"; then fail=$((fail+1)); else pass=$((pass+1)); fi
