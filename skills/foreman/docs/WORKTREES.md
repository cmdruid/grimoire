...
`.agents/foreman/` work scoped to a feature (its plan, its hand-off) rides in that feature's worktree and merges
with it; small standalone `.agents/foreman/` edits -- a backlog entry, a doc tweak -- go straight on `main`. The
load-bearing rule is **no *uncommitted* churn on the shared root**, not "never touch `.agents/foreman/` on
`main`."
...
are additive (config files, registration chains, shared `.agents/foreman/` coordination files) -- resolution is
