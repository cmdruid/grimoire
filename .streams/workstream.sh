...
usage: workstream.sh <checkout> <operation> [args...]
  ...
  sync <stream>
  recycle <stream> [--discard-active] [--source-kind <brief|plan|roadmap>] [--cursor <value>]
  close-check <stream>
  list
  ship-prepare <stream>
...
emit_read_projection() { # stream; requires admitted globals
  ...
  source_kind="$(tracker_get queue - source-kind)"
  source_pointer="$(tracker_get queue - cursor)"
...
cmd_recycle() {
  [ "$#" -ge 1 ] || die "usage: recycle <stream> [--discard-active] [--source-kind <brief|plan|roadmap>] [--cursor <value>]"
  local stream="$1" raw source_kind=brief cursor=- queue_state=intake source_seen=no cursor_seen=no discard_active=no branch target active_unit=
  shift
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --source-kind)
      ...
      --cursor)
        ...
        ;;
      --discard-active)
        [ "$discard_active" = no ] || die "--discard-active may be specified once"
        discard_active=yes; shift
        ;;
  ...
  if [ "$discard_active" = yes ]; then
    [ -z "$(git -C "$WT" status --porcelain)" ] || die "discard-active requires a completely clean worktree"
    ! awk -F '\t' '$1=="shipment" {found=1} END{exit found?0:1}' "$TRACKER" || die "discard-active refuses an active shipment"
    ! awk -F '\t' '$1=="hook"&&$3=="state"&&($4=="ready"||$4=="running"){found=1} END{exit found?0:1}' "$TRACKER" || die "discard-active refuses an unresolved hook"
    active_unit="$(awk -F '\t' '$1=="unit"&&$3=="state"&&$4=="active" {print $2}' "$TRACKER")"
  else
  ...
  awk -F '\t' -v unit="$active_unit" 'NR>1 && !(($1=="queue"&&$2=="-"&&($3=="cursor"||$3=="state"))||($1=="phase"&&$2=="-"&&($3=="name"||$3=="next-action"))||(unit!=""&&$1=="unit"&&$2==unit))' "$TRACKER" >"$raw"
  ...
  if [ "$discard_active" = yes ]; then printf 'discarded_unit=%s\n' "${active_unit:-none}"; fi
}
