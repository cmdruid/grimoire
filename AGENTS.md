...
- If `WORKSTREAM.md` exists at that tree's **top level**, you may be the session driving that
  workstream — STOP before further work, admit its coordinates, then use the effective
  `.streams/workstream.sh ... read <stream>` projection. Reconcile that bounded projection with Git;
  do not read the full runbook or raw tracker.
- If instead a `.streams/<stream>/WORKSTREAM.md` under the top level records
  `isolation<TAB>in-place` **and** HEAD is on that stream's branch, use the same admitted bounded
  projection — you are in the shared tree that stream holds.
