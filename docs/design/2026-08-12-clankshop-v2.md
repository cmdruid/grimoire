...
reserved directories, like the `history.tsv` ledger at the records root, that `records.sh`
skips when scanning stores.
...
status:   # open | current | done | dropped | superseded | consumed
created:  # YYYY-MM-DD
updated:  # YYYY-MM-DD
tags:     # free-form list
```
...
**Closure is in place; history is a ledger.** A finished record never moves (moves would
dangle every path-based link): `records.sh done` sets its closing status — `done`
(completed), `dropped` (won't do), `superseded` (replaced; the note names the successor),
or `consumed` (absorbed into the spec or doctrine; the note names where) — and appends one
line to **`.records/history.tsv`**: `date · disposition · path · doctype · title · note`.
The ledger records *events*, not state — an entry is an immutable fact the moment it is
written, so it cannot drift and does not violate the no-stored-index rule. It is written
only by `records.sh done`, never by hand. Closed records may eventually be *pruned* by the
review station's curation chore (the file deleted; the ledger line and git history remain
the trace) — the prune threshold is project doctrine. Shipped `workstream` units land the
same way: ledger entries, plus a `reports` record tagged `debrief` when the unit warrants
narrative.
...
- `touch` updates the `updated:` stamp (and optionally `status:`) — agents never hand-edit
  dates.
- `done` closes a record in place (default disposition `done`) and appends its ledger line
  to `.records/history.tsv` — the ledger's sole writer.
- `history` scans the ledger — same TSV discipline as `list`.
- `check` is the conformance scan `clankshop check` calls. It also enforces
  status↔ledger coherence: a record with a closing status and no ledger line is a fact.
...
| helper | `journal` (was `backlog`) | **owns the records layer** — capture, curation, completion + the history ledger, escalation, `records.sh`, templates, `.records` scaffolding; stands up records standalone on a bare repo; a *required* pack member (setup delegates to it) |
| helper | `workstream` | long-lived development streams — worktrees, queues, shipping; records shipped units in the history ledger (plus a debrief report when warranted) and takes build-station context when a workshop is present |
...
>   is felt.
> - A stale open record is a lie. Curate: close, supersede, or prune — the ledger keeps
>   the trace; do not let the stores rot.
